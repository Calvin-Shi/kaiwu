...
class GradientFusion(object):
    ...
    def __init__(self):
        ...
        self.grad_to_fp16 = False

        self.use_grad_clip = True

        self.grad_clip_range = 0.5

        self.use_fusion = True

        self.use_xla_fusion = True

        self.piecewise_fusion_schedule = "28;40"

        self.sparse_as_dense = True
    ...
    def run(self, gradvars):
        ...
        if self.grad_to_fp16:
            self.compression = hvd.Compression.fp16

        if self.use_xla_fusion:
            ...
            gradvars, max_noisescale = self._piecewise_fusion(
                self.piecewise_fusion_schedule,
                gradvars,
                self.num_gpus,
                self.batch_size,
                self.compression,
    ...
    def _grad_fusion(self, gradvars, num_gpus, batch_size, compression):
        ...
        if self.use_grad_clip:
            allgrads, _ = tf.clip_by_global_norm(allgrads, self.grad_clip_range)
