...
from kaiwudrl.server.aisrv.predictor_local import PredictorLocal
...
class AsyncPolicy(Policy):
    ...
    def need_train(self):
        ...
        """
        return CONFIG.run_mode == KaiwuDRLDefine.RUN_MODE_TRAIN
    ...
    # aisrv 朝 learner 发送获取训练指标请求
    def get_training_metrics(self, slot_id, msg, agent_ctx):
        agent_id = agent_ctx.agent_id
        learn_index = agent_id % len(self._learner_proxy_list)

        return self._learner_proxy_list[learn_index].get_training_metrics(msg)
    ...
    # 朝learner发送管理流
    def send_control_data(self, control_data, agent_ctx):
        agent_id = agent_ctx.agent_id
        learn_index = agent_id % len(self._learner_proxy_list)

        return self._learner_proxy_list[learn_index].put_data_control(agent_id, control_data)
...
class AsyncBuilder(PolicyBuilder):
    def __init__(self, policy_name, simu_ctx):
        ...
        # 根据配置remote_agent_default_runtime_mode的不同来设置不同的PredictorLocal
        for i in range(0, actor_proxy_num):
            actor_addr = str_to_addr(actor_addrs[i])
            """
            1. 如果是远程预测, 则启动actor_proxy与actor交互
            2. 如果是本机预测, 则：
            2.1 如果是在aisrv里启动预测进程, 则启动predict_local进程
            2.2 如果是在aisrv里的workflow所在的进程预测, 同时需要使用对手模型等功能, 则启动predict_local进程
            2.3 如果是在aisrv里的workflow所在的进程直接函数调用, 则不用启动新的预测进程
            """
            if CONFIG.wrapper_type == KaiwuDRLDefine.WRAPPER_REMOTE:
                if (
                    ...
                    self.actor_proxy_list[i] = PredictorLocal(policy_name, i, actor_addr, simu_ctx)
                elif (
                    CONFIG.remote_agent_default_runtime_mode
            ...
            if self.actor_proxy_list[i]:
                ...
                # 注册到进程健康监控器(如果存在)
                if hasattr(simu_ctx, "process_health_monitor") and simu_ctx.process_health_monitor:
                    # 根据实际进程类型设置不同的名称和类型
                    if isinstance(self.actor_proxy_list[i], PredictorLocal):
                        process_name = f"predictor_{policy_name}_{i}"
                        process_type = "predictor"
                    else:  # ActorProxy
                        process_name = f"actor_proxy_{policy_name}_{i}"
                        process_type = "actor_proxy"

                    simu_ctx.process_health_monitor.register_process(
                        pid=self.actor_proxy_list[i].pid,
                        name=process_name,
        ...
        # 评估场景里不需要启动aisrv, 此时就不需要启动learner, 故此时不需要启动learner_proxy
        if CONFIG.need_to_start_learner:
            # 策略数量
            policy_num = len(CONFIG.learner_addrs)
            if policy_num <= 2:
                ...
                if not (int(CONFIG.self_play) and policy_name == CONFIG.self_play_old_policy):
                    for i in range(0, learner_proxy_num):
                        ...
                        if hasattr(simu_ctx, "process_health_monitor") and simu_ctx.process_health_monitor:
                            simu_ctx.process_health_monitor.register_process(
                                pid=self.learner_proxy_list[i].pid,
                                name=f"learner_proxy_{policy_name}_{i}",
                                process_type="learner_proxy",
                            )

                    if CONFIG.use_sample_server:
                        for i in range(0, CONFIG.sample_server_count):
                            self.sample_sever_list[i] = SampleServer(self.learner_proxy_list, simu_ctx)
                            self.sample_sever_list[i].start()
            else:
                # 当policy大于2个时, 默认只是选择第一个policy即可
