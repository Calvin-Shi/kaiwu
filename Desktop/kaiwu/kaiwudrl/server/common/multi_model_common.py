...
class MultiModelManager(object):
    ...
    def copy_model_files(self):
        ...
        if len(source_model_dir_list) != len(source_model_id_list):
            self.logger.error(
                ...
                f"!= len(source_model_id_list) {len(source_model_id_list)}, configuration error"
            )
            return False, None
    ...
    def init_load_models(self):
        """
        初始化model调用, 主要操作如下:
        1. 根据配置kaiwudrl/conf/kaiwudrl/configure.toml文件里的配置项init_model_file_list将文件从目录拷贝到项目主目录下
        2. 调用MultiModel类加载Agent
        """
        success, result = self.copy_model_files()
        if success:
            # 成功情况下, 但是空文件
            if not result:
                self.logger.info(f"predict no model_pool files configured, init success")
                self.init_success = True
                return
        else:
            # 失败情况
            self.logger.error(f"predict model_pool files configured but copy_model_files failed, init failed")
            self.init_success = False
            return

        # result 为非空字典: 有模型需要加载
        has_error = False
        for model_id, model_info in result.items():
            ...
            if not multi_model.init():
                self.logger.error(f"predict multi_model.init failed for model_id {model_id}, please check")
                has_error = True
                continue

            self.model_id_to_models[str(model_id)] = multi_model
            self.logger.info(f"predict multi_model.init success for model_id {model_id}")

        # 只有所有配置的模型都加载成功才标记为成功
        if has_error:
            self.logger.error(f"predict some models failed to initialize, init failed")
            self.init_success = False
        else:
            self.logger.info(f"predict all {len(self.model_id_to_models)} models initialized successfully")
            self.init_success = True

    def _dispatch(self, method_name, model_id, predict_data):
        """
        通用分发函数，根据 method_name 调用对应 MultiModel 上的方法
        """
        if model_id is None:
            return None
        ...
        if not model_object:
            self.logger.error(f"predict self.model_id_to_models.get failed for model_id {model_id}, please check")
            return None

        handler = getattr(model_object, method_name, None)
        return handler(predict_data) if handler else None

    def predict(self, model_id, predict_data):
        return self._dispatch("predict", model_id, predict_data)

    def exploit(self, model_id, predict_data):
        return self._dispatch("exploit", model_id, predict_data)

    def reset(self, model_id, predict_data):
        return self._dispatch("reset", model_id, predict_data)

    def init_config(self, model_id, predict_data):
        return self._dispatch("init_config", model_id, predict_data)
...
class MultiModel(object):
    ...
    def load_model(self):
        ...
        """
        try:
            self.agent.load_model(self.model_path, self.ckpt_id, framework=True)
    ...
    def _dispatch_agent(self, method_name, predict_data):
        """
        通用分发函数，调用 self.agent 上对应的方法
        """
        if self.agent is None:
            return None
        ...
        handler = getattr(self.agent, method_name, None)
        return handler(predict_data, framework=True) if handler else None

    def predict(self, predict_data):
        return self._dispatch_agent("predict", predict_data)

    def exploit(self, predict_data):
        return self._dispatch_agent("exploit", predict_data)

    def reset(self, predict_data):
        return self._dispatch_agent("reset", predict_data)

    def init_config(self, predict_data):
        return self._dispatch_agent("init_config", predict_data)
