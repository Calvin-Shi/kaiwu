...
class ReverbReplayBuffer(ReplayBufferBase):
    ...
    def _build_limiter(self):
        ...
        if CONFIG.reverb_rate_limiter == "MinSize":
            rate_limiter = reverb.rate_limiters.MinSize(
                int(CONFIG.replay_buffer_capacity * CONFIG.preload_ratio / CONFIG.reverb_table_size)
            )
        else:
            rate_limiter = reverb.rate_limiters.SampleToInsertRatio(
                samples_per_insert=CONFIG.reverb_samples_per_insert,
                min_size_to_sample=int(CONFIG.replay_buffer_capacity * CONFIG.preload_ratio / CONFIG.reverb_table_size),
    ...
    # 采用reverb_client获取随机样本, 调用时注意内存增长情况
    def sample(self, client, batch_size):
        if not client:
            return None
        ...
        batch = [x[0].data[0] for x in data]
        return [batch]
