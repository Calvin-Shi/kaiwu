...
def tar_file_extract(input_file_name, destination_dir):
    ...
    with warnings.catch_warnings():
        ...
        try:
            with tarfile.open(input_file_name, "r") as tar:
                # 安全检查：确保没有文件是绝对路径或包含父目录引用
                safe_files = [
                    member
                    for member in tar.getmembers()
                    if member.isreg() and not member.name.startswith("/") and ".." not in member.name
                ]
...
def actor_learner_aisrv_count(host, srv_name):
    ...
    # 建立连接的TCP数目
    cmd = f"ss -nlt  | grep {host}:{port} | grep ESTAB | wc -l"
...
def compress_data(data, serialize=True):
    ...
    try:
        # 序列化
        if serialize:
            if CONFIG.aisrv_actor_protocol == KaiwuDRLDefine.PROTOCOL_MSGPACK:
...
def decompress_data(data, deserialize=True, uncompressed_size=None):
    ...
    try:
        ...
        if deserialize:
            # 反序列化
            if CONFIG.aisrv_actor_protocol == KaiwuDRLDefine.PROTOCOL_MSGPACK:
                decompress_msg = msgpack.unpackb(decompress_msg, raw=False)
            elif CONFIG.aisrv_actor_protocol == KaiwuDRLDefine.PROTOCOL_PICKLE:
...
def write_json_to_file(data_dict, file_name, target_dir):
    file_path = target_dir + "/" + file_name + ".json"
    with open(file_path, "w") as json_file:
        json_file.write(json.dumps(data_dict))
        json_file.flush()  # 强制刷新缓冲区
        os.fsync(json_file.fileno())  # 确保数据写入磁盘
...
def compute_directory_hash(dir_path, exclude_files=None):
    ...
    for root, dirs, files in os.walk(dir_path):
        for file in files:
            ...
            # 如果这个文件是包含了model.ckpt的则需要返回
            if KaiwuDRLDefine.KAIWU_MODEL_CKPT in file:
    ...
    return sha256_hash.hexdigest(), model_files
...
def public_signature_verify_by_dir(public_key, data, private_signature):
    """
...
def private_signature_verify_by_dir(private_key, data, public_signature):
    """
...
def get_machine_device_by_config(framework_name, process_name):
    ...
    if framework_name == KaiwuDRLDefine.DEEP_LEARNING_FRAMEWORK_PYTORCH:
        ...
        framework_name == KaiwuDRLDefine.DEEP_LEARNING_FRAMEWORK_TENSORFLOW_COMPLEX
        or framework_name == KaiwuDRLDefine.DEEP_LEARNING_FRAMEWORK_TENSORFLOW_SIMPLE
    ):

        return None

    elif framework_name == KaiwuDRLDefine.DEEP_LEARNING_FRAMEWORK_TCNN:
        return None

    elif framework_name == KaiwuDRLDefine.DEEP_LEARNING_FRAMEWORK_TENSORRT:
        return None
...
def scan_for_errors(directory, error_indicator="ERROR"):
    """
    Scan for error logs in a directory
    扫描某个文件夹下的错误日志
    """
    for root, dirs, files in os.walk(directory):
        for filename in files:
            ...
            try:
                # Force using utf-8 encoding with error handling to avoid encoding issues
                # 强制使用 utf-8 编码并配合错误处理，避免编码问题
                ...
                # Skip files that cannot be read or are directories
                # 跳过无法读取的文件或目录
