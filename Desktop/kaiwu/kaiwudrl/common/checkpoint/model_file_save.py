...
class ModelFileSave(Worker):
    ...
    def push_file_to_cos(self, temp_remote_dirs):
        ...
        for key, local_and_remote_dir in temp_remote_dirs.items():
            if MODEL_PATH_MANGER.need_to_sync(key):
                self.logger.info(
                    f"model_file_save local_and_remote_dir {key}/{local_and_remote_dir} need to sync to COS",
                    g_not_server_label,
                )
                ...
                try:
                    ...
                    if CONFIG.use_bypass:
                        # time.time()返回的是以秒为单位
                        now = int(time.time())
                        if now - self.last_bypass_time >= CONFIG.bypass_per_minutes * 60:
    ...
    def before_run(self):
        ...
        self.logger.info(
            f"model_file_save process start success at pid {self.current_pid}",
            g_not_server_label,
        )
    ...
    def multi_replace_eval_model_dir(self):
        ...
        if ckpt_file_list:
            ...
            if (
                KaiwuDRLDefine.DEEP_LEARNING_FRAMEWORK_TENSORFLOW_COMPLEX == CONFIG.use_which_deep_learning_framework
                or KaiwuDRLDefine.DEEP_LEARNING_FRAMEWORK_TENSORFLOW_SIMPLE == CONFIG.use_which_deep_learning_framework
                ...
                KaiwuDRLDefine.DEEP_LEARNING_FRAMEWORK_PYTORCH == CONFIG.use_which_deep_learning_framework
                or KaiwuDRLDefine.NO_DEEP_LEARNING_FRAMEWORK == CONFIG.use_which_deep_learning_framework
            ):
                # pytorch指定到具体的文件
    ...
    def process_model_file(self, model_file):
        ...
        if model_file not in self.model_file_list:
            ...
            json_file = (
                f"{os.path.dirname(model_file)}/"
                f"{KaiwuDRLDefine.KAIWUDRL_MODEL_FILE_JSON_FILE_NAME}_"
                f"{checkpoint_id}.json"
            )

            # 增加重试机制，防止learner进程还在写json文件
            max_retry = 3
            retry_count = 0
            ...
            while retry_count < max_retry:
                if os.path.exists(json_file):
                    ...
                    if retry_count < max_retry:
                        self.logger.warning(
                            f"model_file_save source json file {json_file} not found, retry {retry_count}/{max_retry}",
                            g_not_server_label,
                        )
                        time.sleep(1)  # 等待1秒
            ...
            if json_file_found:
                ...
                # json 文件不存在，无法继续处理
                self.logger.warning(
                    f"model_file_save source json file {json_file} not found after {max_retry} retries, cannot process model file",
                    g_not_server_label,
                )
                return False
            ...
            if not os.path.exists(cos_local_target_dir_kaiwu_json):
                self.logger.warning(
                    f"model_file_save cos_local_target_dir_kaiwu_json {cos_local_target_dir_kaiwu_json} "
                    f"not found, please check",
                    g_not_server_label,
                )
                return False
            ...
            try:
                ...
                # json文件可能在learner进程被kill时写到一半，导致格式损坏
                self.logger.warning(
                    f"model_file_save failed to parse json file {cos_local_target_dir_kaiwu_json}, "
                    f"file may be corrupted, error: {str(e)}",
                    g_not_server_label,
                )
                # 删除损坏的json文件
                if os.path.exists(cos_local_target_dir_kaiwu_json):
            ...
            # 计算模型文件的hash值
            ckpt_dir_hash, model_files = compute_directory_hash(f"{CONFIG.cos_local_target_dir}/ckpt")
            data["model_file_hash"] = ckpt_dir_hash

            # 写入model_file_path, 注意多个文件采用数组形式, 需要加上相对路径
            model_files = ["ckpt/" + filename for filename in model_files]
            data["model_file_path"] = model_files
            ...
            if reward_config:  # 只有非空字符串才成立
                data["reward_config"] = f"{reward_config}_runtime_{train_time}"
            ...
            cos_local_target_dir_ckpt_kaiwu_json = (
                f"{CONFIG.cos_local_target_dir}/ckpt/{KaiwuDRLDefine.KAIWUDRL_MODEL_FILE_JSON_FILE_NAME}.json"
            )
            ...
            # 整个打包成zip, 排除掉不需要打包的文件内容
            folder_path = f"conf/kaiwudrl"
            python_exec_shell(
                f"cd {CONFIG.cos_local_target_dir} && zip -r -q {zip_file_name} . -x '{folder_path}/*' -x '{folder_path}/'"
            )
    ...
    def save_model_file_to_cos_standard(self):
        ...
        try:
            ...
            for model_file in model_file_list:
                model_file_stripped = model_file.strip()

                if self.process_model_file(model_file_stripped):
                    # 统计值增加
                    self.push_to_cos_succ_cnt += 1
                else:
                    # 统计值增加
                    ...
                    # 即使处理失败，也要标记为已处理，避免重复尝试同一个文件
                    # 这样可以避免在容器删除时死循环打印同一个错误
                    if model_file_stripped not in self.model_file_list:
                        self.model_file_list.append(model_file_stripped)
    ...
    def run_once(self):
        ...
        # 检查是否需要保存模型到 COS
        if now - self.last_run_schedule_time_by_to_cos > CONFIG.model_file_save_per_minutes * 60:
            # 先更新时间戳，防止高频循环下重复执行
            self.last_run_schedule_time_by_to_cos = now
            self.save_model_file_to_cos()

        # 检查是否需要上报监控统计
        if now - self.last_run_schedule_time_by_save_stat > CONFIG.prometheus_stat_per_minutes * 60:
            # 先更新时间戳，防止高频循环下重复执行
    ...
    def run(self) -> None:
        ...
        while not self.exit_flag.value:
            try:
                ...
                self.logger.exception(
                    f"model_file_save run_once, err: {str(e)}",
                    g_not_server_label,
                )
    ...
    def start_actor_process_by_type(self, logger):
        ...
        if not CONFIG.start_actor_learner_process_type:
            logger.info(
                f"model_file_save process start, type is {CONFIG.start_actor_learner_process_type}, "
    ...
    def handle_sigterm(self, sig, frame):
        # SIGTERM信号处理，等待5s让learner进程有时间完成文件写入，然后每2s手动尝试上传模型，直到被强制退出
        time.sleep(5)  # 增加等待时间，让learner进程有时间完成json文件写入
        self.logger.info(f"model_file_save {self.current_pid} start handle SIGTERM.")
        while True:
            try:
                self.save_model_file_to_cos()
                self.model_file_save_stat()
