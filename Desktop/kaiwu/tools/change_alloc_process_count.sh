...
if [ $# -ne 2 ];
then
    echo -e "\033[31m useage: sh kaiwudrl/utils/change_alloc_process_count.sh aisrv|actor|learner|kaiwu_env count \
    such as: sh kaiwudrl/utils/change_alloc_process_count.sh aisrv 1  \033[0m"
...
then
    ...
    sed -i "s/aisrv_connect_to_actor_count = .*/aisrv_connect_to_actor_count = $count/g" $project_default_configure_file
...
then
    ...
    sed -i "s/aisrv_connect_to_learner_count = .*/aisrv_connect_to_learner_count = $count/g" $project_default_configure_file
...
then
    ...
    sed -i "s/aisrv_connect_to_kaiwu_env_count = .*/aisrv_connect_to_kaiwu_env_count = $count/g" $project_default_configure_file
