...
if [ $# -ne 1 ];
then
    echo -e "\033[31m useage: sh tools/change_sample_server.sh reverb|zmq|file_mmap|shared_memory, sh tools/change_sample_server.sh reverb  \033[0m"
...
# 下面是具体的修改配置文件的操作
learner_configure_file="kaiwudrl/conf/kaiwudrl/learner.toml"
...
elif [ $sample_server_type == "file_mmap" ];
then
    sed -i 's/use_learner_server = .*/use_learner_server = false/' $learner_configure_file
    sed -i 's/replay_buffer_type = .*/replay_buffer_type = "file_mmap"/' $configure_file

elif [ $sample_server_type == "shared_memory" ];
then
    sed -i 's/use_learner_server = .*/use_learner_server = false/' $learner_configure_file
    sed -i 's/replay_buffer_type = .*/replay_buffer_type = "shared_memory"/' $configure_file
