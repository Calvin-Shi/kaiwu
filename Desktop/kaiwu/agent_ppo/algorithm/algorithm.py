...
class Algorithm:
    ...
    def learn(self, list_sample_data):
        _input_datas = torch.stack([t.npdata for t in list_sample_data], dim=0).to(self.device)
        results = {}
        ...
        SERI_DIM = self.seri_vec_split_shape[0][0]
        feature_vec = feature.reshape(-1, SERI_DIM).to(self.device)          # (B*T, D)

        lstm_hidden_state = init_lstm_hidden.reshape(-1, self.lstm_unit_size).to(self.device) 
        lstm_cell_state = init_lstm_cell.reshape(-1, self.lstm_unit_size).to(self.device) 
