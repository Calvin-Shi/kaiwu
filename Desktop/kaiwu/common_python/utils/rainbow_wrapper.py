...
class RainbowWrapper(object):
    ...
    def __init__(self, rainbow_url, app_id, user_id, secret_key, env_name, logger):
        ...
        self.rainbow_app_id = app_id
        self.rainbow_user_id = user_id
        self.rainbow_secret_key = secret_key
        self.rainbow_env_name = env_name
        self.logger = logger

        self.rainbow_utils = RainbowUtils(
            self.rainbow_url,
            self.rainbow_app_id,
            self.rainbow_user_id,
            self.rainbow_secret_key,
            self.rainbow_env_name,
            self.logger,
        )

        self.logger.info(f" RainbowUtils {self.rainbow_utils.identity}")
    ...
    def rainbow_activate_single_process(self, process_name):
        ...
        if not int(CONFIG.use_rainbow):
            self.logger.info(f" CONFIG.use_rainbow is False, so return")
            return

        result_code, data, result_msg = self.rainbow_utils.read_from_rainbow(process_name)
        if result_code:
            self.logger.error(
                f"read_from_rainbow failed, process_name is {process_name}, msg is {result_msg}, result_code is {result_code}"
            )
            return

        if not data or not len(data):
            self.logger.error(
                f"read_from_rainbow failed, process_name is {process_name}, data is None or data len is 0"
            )
            return
        ...
        self.logger.info(f" {process_name} CONFIG process_name is {process_name}, save_to_file success")
