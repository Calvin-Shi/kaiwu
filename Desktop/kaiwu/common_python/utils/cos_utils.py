...
class COSSave(object):
    ...
    def push_to_cos(self, file_name, bucket_name, key):
        ...
        with open(file_name, "rb") as fp:
            try:
                ...
                if self.logger:
                    self.logger.error(
                        f"cos error to push to cos file_name {file_name} bucket_name {bucket_name}  key {key} error is {str(e)}"
                    )
    ...
    def query_object_list(self, bucket_name, prefix):
        ...
        try:
            ...
            if self.logger:
                self.logger.error(
                    f"cos error to query object list, bucket_nbame {bucket_name} prefix {prefix}, error is {str(e)}"
                )
    ...
    def get_from_cos(self, bucket_name, key, output_file_name):
        ...
        try:
            ...
            if self.logger:
                self.logger.error(
                    f"cos error to get from cos, bucket_nbame {bucket_name} key {key} output_file_name {output_file_name} error is {str(e)}"
                )
    ...
    def delete_from_cos(self, bucket_name, key):
        ...
        try:
            ...
            if self.logger:
                self.logger.error(
                    f"cos error to delete from cos, bucket_nbame {bucket_name} key {key} error is {str(e)}"
                )
