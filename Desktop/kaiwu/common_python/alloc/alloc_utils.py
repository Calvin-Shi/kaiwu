...
from common_python.utils.http_utils import http_utils_post, http_utils_request
...
class AllocUtils(object):
    ...
    def __init__(self, logger) -> None:
        ...
        self.SERVER_PORT_CONFIGURE = {
            CommonDefine.SERVER_AISRV: int(CONFIG.aisrv_server_port),
            CommonDefine.SERVER_ACTOR: int(CONFIG.zmq_server_port),
            CommonDefine.SERVER_LEARNER: int(CONFIG.reverb_svr_port),
            CommonDefine.SERVER_CLIENT: int(CONFIG.client_svr_port),
            CommonDefine.SERVER_ENV: int(CONFIG.kaiwu_env_svr_port),
        }
        ...
        self.SERVER_ASSIGN_LIMIT = {
            CommonDefine.SERVER_AISRV: int(CONFIG.alloc_process_assign_limit_aisrv),
            CommonDefine.SERVER_ACTOR: int(CONFIG.alloc_process_assign_limit_actor),
            CommonDefine.SERVER_LEARNER: int(CONFIG.alloc_process_assign_limit_learner),
            CommonDefine.SERVER_CLIENT: int(CONFIG.alloc_process_assign_limit_client),
            CommonDefine.SERVER_ENV: int(CONFIG.alloc_process_assign_limit_kaiwu_env),
        }
    ...
    def get_server_process_count(self, srv_name):
        ...
        if srv_name == CommonDefine.SERVER_ACTOR:
            count = int(CONFIG.aisrv_connect_to_actor_count)
        elif srv_name == CommonDefine.SERVER_LEARNER:
            count = int(CONFIG.aisrv_connect_to_learner_count)
        elif srv_name == CommonDefine.SERVER_AISRV:
            count = 1
        elif srv_name == CommonDefine.SERVER_ENV:
    ...
    def get(self, srv_name, set_name, self_play_set_name):
        ...
        if not int(CONFIG.self_play):
            ...
            if set_name != self_play_set_name:
                if target_role == self.SERVER_ROLE_CONFIGURE.get(CommonDefine.SERVER_ACTOR):
                    for i in range(int(count)):
    ...
    def get_actor_learner_ip(self, set_name, self_play_set_name):
        ...
        if not int(CONFIG.self_play):
            ...
            if code:
                ...
                ) = self.get_actor_learner_address(content, self.SERVER_ROLE_CONFIGURE[CommonDefine.SERVER_ACTOR])
            ...
            if code:
                ...
                ) = self.get_actor_learner_address(content, self.SERVER_ROLE_CONFIGURE[CommonDefine.SERVER_LEARNER])
    ...
    def get_aisrv_ip(self, set_name, self_play_set_name) -> list:
        ...
        # 获取aisrv地址
        aisrv_address = []
        code, msg, content = self.get(CommonDefine.SERVER_AISRV, set_name, self_play_set_name)
        if code:
    ...
    def get_kaiwu_env_ip(self, set_name, self_play_set_name) -> list:
        ...
        # 获取kaiwu_env地址
        kaiwu_env_address = []
        code, msg, content = self.get(CommonDefine.SERVER_ENV, set_name, self_play_set_name)
        if code:
